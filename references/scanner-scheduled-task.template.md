...
over LinkedIn + Wellfound, the keyless API aggregators, two-stage scoring, a
live-status check, and the canonical-ATS lookup. The browser pass needs the **Claude
for Chrome extension** and the user signed in to those sites; if Chrome isn't
available when the task fires, the task skips that pass and continues with the API
sources.
...
description: Scheduled job scan — logged-in browser pass (LinkedIn + Wellfound) plus keyless API aggregators, scored against the user's criteria; surfaces a ranked shortlist.
---
...
tab:
  ...
  CAPTURE DEEP, not just the first screen: one page-read returns only the top few
  results. After the first read, SCROLL the results pane, wait ~1s, read again,
  repeat ~8-10x (on LinkedIn also page to 2-3). Accumulate ALL unique listings; stop
  when clearly older than 24h (LinkedIn) or after ~75 rows (Wellfound). Keep only
  genuine matches for the user's target role (use their role_keywords/levels); drop
  keyword noise. Write kept rows as JSON arrays of
  {title, company, location, url, comp_text, description} to
  fetched/linkedin__<role-slug>.json and fetched/wellfound__<role-slug>.json.

=== PASS 2: API AGGREGATORS ===
  ...
  2. Fetch each, save raw JSON to fetched/{source}__{label}.json (web tool; if a
     fetch is provenance-blocked, fetch via the Chrome tools). Skip failures.
...
=== STAGE 1: CARD-LEVEL SCORE (every role) ===
  3. `python3 [SKILL_DIR]/scripts/job_scanner.py --input-dir fetched --config [WORKING_FOLDER]/user-library/scanner_config.json`
     ...
     0-100, writes shortlist_latest.json, and prints a ranked markdown block. This is
     the cheap pass — it rules out roughly half on obvious comp/domain mismatch. Each
     item is flagged deep_read=true/false against `deep_read_threshold` (default 50).

=== STAGE 2: DEEP READ (only the flagged cards) ===
  4. Do NOT deep-read everything. For each role with deep_read=true, open its `url`
     and read the FULL JD (web tool; if blocked or JS-heavy, use the Chrome page-text
     reader).
...
=== STAGE 2b: CANONICAL ATS LINK (for profile-only finds) ===
  4b. For each shortlisted role whose `url` is a profile-only source that blocks file
     upload (wellfound.com or a linkedin.com job listing), do ONE web search like
     `"<Company>" "<Title>" greenhouse OR ashby OR lever` (and/or check the company
     careers page) to find the CANONICAL ATS posting where the tailored résumé + cover
     letter can actually be uploaded. If found and it's the same role, capture it as
     the "Apply at" link. If none exists, note "profile-only — no ATS posting found."
     One or two searches per role, no more.
